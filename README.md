...
- **Tourist app** (`index.html`): boarding-screen island picker with a small illustrative atoll map, category-filtered listings that pop up full detail on tap, an events tab, and the $2 Pro unlock (contact details + full events calendar) — all in 8 languages, boarding-screen and app-chrome only. Desktop layout (≥860px) switches to a wider grid with the tabs moved to the top.
