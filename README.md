...
| File | What it is |
|---|---|
| `index.html` | The tourist app (island picker, Explore, Events) |
| `business.html` | The standalone Business Portal — its own URL, separate from the tourist app |
...
Before pushing (or as a follow-up commit), edit one line in **all three** of `index.html`, `business.html`, and `superadmin.html` — search for `API_BASE` near the top of the `<script>` tag:
...
2. Open `business.html`, create a test account, add a listing, confirm it shows up in `index.html`'s Explore.
